...
for pt = 1 : 1 %patients
    ...
    nepoch = size(data,3);
    reconErrorTrain = NaN(length(nstacks), length(r_vals), nepoch);
    reconErrorTest = NaN(length(nstacks), length(r_vals), nepoch);
        ...
        for r = 1:length(r_vals)
            ...
            for n = 1:length(nstacks)
                ...
                for ii = 1:size(Xaug,3)
                    [Xdmd_train(:,:,ii), tempErr(:,ii)] = predictDMD(DmdStruct, Xaug(:,:,ii), nstacks);
                end
                reconErrorTrain(n, r, k, :) = nanmean(tempErr);
                
                tempErr = NaN(1,size(Xaug_test,3));
                for ii = 1:size(Xaug_test,3)
                    [Xdmd_test(:,:,ii), tempErr(:,ii)] = predictDMD(DmdStruct, Xaug_test(:,:,ii), nstacks);
                end
                reconErrorTest(n, r, k, :) = nanmean(tempErr,2);
                
                X = revTimeShiftEmbedding(Xaug, nstacks);
                X = revTimeShiftEmbedding(Xaug, nstacks);
                X = revTimeShiftEmbedding(Xaug, nstacks);
                ...
                plotTrainTest(reshape(Xaug(:,:,k-nPl:k-1), size(Xaug,1), []), reshape(Xaug_test, size(Xaug,1), []),...
                    ...
                    pt,k,nstacks(n),r_vals(r),reconErrorTrain(n, r, k),...
                    reconErrorTest(n, r, k),plot_path);
